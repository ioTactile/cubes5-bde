...
const formattedValue = computed(() => props.modelValue !== undefined ? props.modelValue : undefined)
const customRules = computed(() => {
  const result: ((v?: number) => string | true)[] = [...(props.rules || [])]

  if (props.min !== null && props.min !== undefined) {
    // @ts-ignore
    result.push((v: number) => v >= props.min || `La valeur doit être supérieure ou égale à ${props.min}`)
  }
  if (props.max !== null && props.max !== undefined) {
    // @ts-ignore
    result.push((v: number) => v <= props.max || `La valeur doit être inférieure ou égale à ${props.max}`)
  }
...
watch(
  () => props.modelValue,
  (val) => {
    if (typeof val === 'string') { emits('update:model-value', toInt(val)) }
  }
)

let minus = false
const updateValue = (newValue?: string) => {
  if (newValue === '' || newValue === null) { return emits('update:model-value', undefined) }
  if (newValue === '-') {
    minus = true
    ...
    minus = false
    if (isNaN(value)) { return emits('update:model-value', props.modelValue as number|undefined) }
...
const isNumber = (evt: KeyboardEvent) => {
  ...
  if (
    ...
    charCode !== 45 &&
    charCode !== 44
  ) { evt.preventDefault() } else { return true }
}
</script>
